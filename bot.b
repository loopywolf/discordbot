...
function getNickname(myUser, myGuildID) {
  var nickname = client.guilds
    .find("id", myGuildID)
    .members.find("id", myUser.id).nickname;
  if (nickname == null) {
  ...
  if (!gm(message)) {
    ...
    if (parameters.length != 1) {
      message.channel.send(
        "groupinjury: expecting 1 parameter (groupinjury <game>)"
      );
      return 0;
    }
  }
...
function injury(message, dbpool, s = "") {
  ...
  if (s == "") {
    ...
    if (parameters.length != 1) {
      message.channel.send(
        "injury: expecting 1 parameter (groupinjury <character>)"
      );
      return 0;
    }
    sql =
  ...
  dbpool.query(sql, function (err, result, fields) {
    ...
    if (Object.keys(result).length) {
      ...
      cid = result[0].charid;
      sql =
        "SELECT statName,statValue from dice_stats WHERE char_id = '" +
        cid +
        "' AND (statName = 'PW' OR statName = 'PB')";
      result = {};
      dbpool.query(sql, function (err, result, fields) {
        if (err) throw err;
        if (Object.keys(result).length) {
          ...
          if (pcwound >= 100) {
            message.channel.send(
              "injury: " + pnick + " has " + pcwound + "% wounds and is dying."
            );
            ...
            message.channel.send(
              "injury: " +
                ...
                " has " +
                pcwound +
                "% wounds and is injured."
            );
          } else if (pcbody > 0) {
            message.channel.send(
              "injury: " + pnick + " has " + pcbody + "% body and is bruised."
            );
          } else {
            message.channel.send(
              "injury: " + pnick + " does not have any injuries."
            );
          }
        } else {
          message.channel.send(
...
function luck(message, dbpool, s) {
  ...
  if (!gm(message)) {
    ...
    if (parameters.length != 2) {
      message.channel.send(
        "luck: expecting 2 parameters (luck <character> <amount>)"
      );
      return false;
    }
    ...
    if (isNaN(charluck)) {
      message.channel.send(
        "luck: expecting numerical value for luck (luck <character> <amount>)"
      );
      return false;
    }
    sql =
...
function setvalue(message, dbpool, s) {
  //updated to .setvalue Adept BRN = 6
  s = mysql_real_escape_string(s);
  var cid = 0;
  var pnick = "";
  var value = 0;
  ...
  if (!gm(message)) {
    ...
    if (parameters.length != 4) {
      message.channel.send(
        "setvalue: expecting 4 parameters (setvalue <character> <stat> = <value>)"
      );
      return false;
    }
    if (parameters[2] != "=") {
    ...
    if (isNaN(value)) {
      message.channel.send(
        "setvalue: expecting numerical value for value (setvalue <character> <stat> <value>)"
      );
      return false;
    }
    sql =
    ...
    dbpool.query(sql, function (err, result, fields) {
      ...
      if (Object.keys(result).length) {
        cid = result[0].charid;
        sql =
          "INSERT INTO dice_stats (char_id, statName, statValue, tally) VALUES ('" +
          cid +
          "','" +
          statName +
          "'," +
        ...
        pnick = result[0].nickname;
        sql =
          "UPDATE dice_stats SET statValue = '" +
          value +
          "' WHERE dice_stats.char_id = '" +
          cid +
          "' AND dice_stats.statName = '" +
          statName +
          "'";
        //console.log("sql="+sql);
        result = {};
        dbpool.query(sql, function (err, result, fields) {
          if (err) throw err;
          message.channel.send(
            "setvalue: " + statName + " set to " + value + " for " + pnick
          );
        });
      } else {
...
function xp(message, dbpool, s) {
  ...
  if (!gm(message)) {
    ...
    if (parameters.length != 2) {
      message.channel.send(
        "xp: expecting 2 parameters (luck <character> <amount>)"
      );
      return false;
    }
    ...
    if (isNaN(charxp)) {
      message.channel.send(
        "xp: expecting numerical value for luck (luck <character> <amount>)"
      );
      return false;
    }
    sql =
...
function bonus(message, dbpool, s) {
  ...
  if (!gm(message)) {
    ...
    if (parameters.length != 2) {
      message.channel.send(
        "bonus: expecting 2 parameters (bonus <character> <amount>)"
      );
      return false;
    }
    ...
    if (isNaN(charbonus)) {
      message.channel.send(
        "bonus: expecting numerical value for bonus (bonus <character> <amount>)"
      );
      return false;
    }
    sql =
    ...
    dbpool.query(sql, function (err, result, fields) {
      ...
      if (Object.keys(result).length) {
        ...
        sql =
          ...
          "' WHERE dice_stats.char_id = '" +
          cid +
          "' AND dice_stats.statName = 'BONUS'";
        result = {};
        dbpool.query(sql, function (err, result, fields) {
          if (err) throw err;
          message.channel.send(
            "bonus: " + pnick + " received " + charbonus + " points."
          );
        });
      } else {
...
function registercharacter(message, s, dbpool) {
  ...
  if (!gm(message)) {
    ...
    if (parameters.length != 3) {
      message.channel.send(
        "add: expecting 3 parameters (add <character> in <game>)"
      );
      return false;
    }
    if (parameters[1] != "in") {
    ...
    dbpool.query(sql, function (err, result, fields) {
      ...
      if (Object.keys(result).length) {
        ...
        result = {};
        sql =
          "INSERT INTO dice_characters (name,game_id,aspects,image_path,map_x,map_y) VALUES ('" +
          nickname +
          "','" +
          gameid +
          "','','','','')";
            ...
            switch (statName) {
              ...
              default:
                ...
                if (tally.startsWith("BASE")) {
                  ...
                  tally.startsWith("BLAST") ||
                  tally.startsWith("DAM") ||
                  tally.startsWith("WPN") ||
                  tally.startsWith("ARMOR") ||
                  tally.startsWith("TFF") ||
                  tally.startsWith("H2H")
                ) {
                  combat[k] =
                    pad(result[i].statValue, 3, " ") +
                    " " +
                    statName +
                    " [" +
                    tally +
                  ...
                  statName.startsWith("IP-") ||
                  statName.startsWith("NIP-") ||
                  statName.startsWith("INCOME")
                ) {
                  ip[o] =
                    pad(result[i].statValue, 2, " ") +
                    " " +
                  ...
                  !statName.startsWith("SD") &&
                  !statName.startsWith("BODY") &&
                  !statName.startsWith("DAMAGE") &&
                  !statName.startsWith("FREEB") &&
                  !statName.startsWith("SUSTAIN") &&
                  !statName.startsWith("LW") &&
                  !statName.startsWith("LB") &&
                  !statName.startsWith("INJURY") &&
                  !statName.startsWith("INIT") &&
                  !statName.startsWith("IO") &&
                  !statName.startsWith("PB") &&
                  !statName.startsWith("PW") &&
                  !statName.startsWith("FREEBIES")
                ) {
                  sp[n] =
                    pad(result[i].statValue, 4, " ") +
                    " " +
                    statName +
                    " [" +
                    tally +
...
`;
          ...
          message.channel.send(
            "https://dstory.games/dstory/sheet.php?name=" + pnick
          );
        }
      });
    } else {
...
function callten(message, dbpool) {
  ...
  dbpool.query(sql, function (err, result, fields) {
    ...
    if (Object.keys(result).length) {
      ...
      cid = result[0].charid;
      sql =
        "SELECT statValue from dice_stats WHERE char_id = '" +
        cid +
        "' AND statName = 'LUCK'";
      console.log(pnick);
      console.log(cid);
        ...
        if (Object.keys(result).length) {
          ...
          if (luck > 0) {
            ...
            dbpool.query(sql, function (err, result, fields) {
              if (err) throw err;
              message.channel.send(
                "call10: " + pnick + " now has luck " + luck
              );
              return;
            });
          } else {
            message.channel.send(
              "call10: " + pnick + " does not have enough luck."
            );
          }
        }
      });
        ...
        if (Object.keys(result).length) {
          ...
          if (luck > 1) {
            ...
            dbpool.query(sql, function (err, result, fields) {
              if (err) throw err;
              message.channel.send(
                "reroll: " + pnick + " now has luck " + luck
              );
              return;
            });
          } else {
            message.channel.send(
              "reroll: " + pnick + " does not have enough luck."
            );
          }
        }
      });
...
function roll(
  ...
  if (diffparse.length != difforig.length) {
    message.channel.send("roll: " + parameters[1] + " isn't a number.");
    return;
  } else {
...
function palindrome(
  ...
  //33 is the same backwards - I'm retiring this
  var rem,
    temp,
    final = 0;
  var finalresult = 0;
  ...
  if (final == temp && temp > 10 && false) {
    //number is the same backwards as forwards - shorted out for time being
    newcrit = crit + dice;
    crittrue++;
    ...
    message.channel.send(
      "roll: " + user + " rolled " + s + " and got " + temp + "/100 = " + result
    );
  }
}
...
function calc(message, s) {
  ...
  if (parameters.length != 5) {
    message.channel.send(
      "roll: I'm expecting 3 parameters like <roll 8 vs 5 with 80>"
    );
    return;
  }
  if (parameters[1] != "v" && parameters[1] != "vs") {
    message.channel.send(
      "roll: I'm expecting a 'vs' as 2nd parameter, like <roll 8 vs 5 with 80>, but I got " +
        parameters[1]
    );
    return;
  }
  ...
  console.log(
    "dice=" + dice + " difficulty=" + difficulty + " roll=" + newroll
  );
...
function catid(message) {
  ...
  message.channel.send(
    "catid: " + user + " checked " + channel + " with parent " + id
  );
}
...
function rollWithStat(
  ...
  statparse.forEach(async function (item, index) {
    ...
    if (isNaN(item)) {
      ...
      console.log(
        "rollWithStat: trying username " + user + " as chr for " + item
      );
      sql =
        "SELECT ds.statValue " +
      ...
      if (result && result.length > 0) {
        ...
        message.channel.send(
          user + "'s " + item + " is " + result[0].statValue
        );
...
function sendStatResult(
  ...
  if (isNaN(difficulty)) {
    message.channel.send(
      "sendStatResult: " + parameters[2] + " isn't a number."
    );
    return;
  }
  console.log("dice=" + dice + " difficulty=" + difficulty);
  ...
  if (gm(message)) {
    ...
    for (var i = 1; i < parameters.length; i++) {
      ...
      if (breakdown.length != 2) {
        message.channel.send(
          "weeklyxp: expecting 2 parameters (e.g. in-game=2)"
        );
        return false;
      }
      var cat = breakdown[0];
      ...
      if (isNaN(award)) {
        message.channel.send(
          "weeklyxp: expecting numerical value for xp (e.g. in-game=2)"
        );
        return false;
      }
      weeklyXp += award;
...
`;
  ...
  } else if (cmd.startsWith("icanplay")) {
